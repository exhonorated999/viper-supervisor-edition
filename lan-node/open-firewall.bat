...
netsh advfirewall firewall add rule name="VIPER LAN Node 7071 TCP" dir=in action=allow protocol=TCP localport=7071 profile=private,domain
netsh advfirewall firewall add rule name="VIPER LAN Node 7071 UDP" dir=in action=allow protocol=UDP localport=7071 profile=private,domain

if %errorlevel%==0 (
  echo.
  echo   Done. Inbound TCP+UDP 7071 allowed on Private/Domain networks.
