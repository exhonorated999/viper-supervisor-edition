// ---------------------------------------------------------------------------
// Canonical dataset owned by the LAN node (stands in for an investigator
// V.I.P.E.R. device / LAN hub). Mirrors the shapes in src/types.ts. In a real
// deployment this data originates on investigator devices; here it lives on the
// node and is served to the supervisor over the encrypted handshake.
// ---------------------------------------------------------------------------
...
export function buildDataset() {
  return {
    ...
    unit: { name: "Major Crimes Unit" },
  };
}

// Pools used to synthesize live events (real-time sync demo).
export const liveEventPool = {
  opsPlans: [
    { title: "Riverside Interdiction", detective: "Det. L. Chen", risk: "High Risk", summary: "Vehicle interdiction on Route 9 corridor. Two-unit coordinated stop with K-9 support requested." },
    { title: "Northgate Buy-Bust", detective: "Det. M. Allen", risk: "Medium Risk", summary: "Buy-bust at Northgate transit center. CI insertion, plainclothes arrest team on standby." },
    { title: "Harbor Watch", detective: "Det. K. Johnson", risk: "Low Risk", summary: "48-hour static observation of dockside warehouse. Documentation only." },
  ],
  activity: [
    { detective: "Det. S. Williams", lastActivity: "Evidence Logged", lastActivityKind: "Evidence" },
    { detective: "Det. J. Martinez", lastActivity: "Warrant Authored", lastActivityKind: "Warrant" },
    { detective: "Det. L. Chen", lastActivity: "Arrest Made", lastActivityKind: "Arrest" },
  ],
};
